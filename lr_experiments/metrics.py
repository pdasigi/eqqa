...
def add_bleu_hf(mocha_dataset, order: int=4, prefix="hf_"):
    ...
    for dataset, examples in mocha_dataset.items():
        ...
        print(f"Found {len(invalid_examples)} invalid examples (according to HF_BLEU) in {dataset}:", invalid_examples)
...
def add_word_movers_distance(mocha_dataset, w2vec_path):
    ...
    for dataset, examples in mocha_dataset.items():
        for example in examples.values():
            ...
            example["wmd-no-stopwords"] = score

            candidate = normalize_answer(example["candidate"], remove_stopwords=False)
            reference = normalize_answer(example["reference"], remove_stopwords=False)
            score = model.wmdistance(candidate, reference)
            example["wmd"] = score
...
def add_input_statistics(mocha_dataset):
    ...
    def get_statistics(text, prefix):
        norm_text = normalize_answer(text).split()

        stats = {
            "length_word": len(norm_text),
            "length_char": len(''.join(norm_text)),
            "nunique_words": len(set(norm_text)),
        }
    ...
    for dataset, examples in mocha_dataset.items():
        for example in examples.values():
            for col in ("candidate", "reference", "context", "question"):
                example.update(get_statistics(example[col], col))
