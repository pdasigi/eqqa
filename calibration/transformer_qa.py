...
@Model.register("transformer_qa_k_best")
class TransformerQA(Model):
    ...
    def __init__(
        self, vocab: Vocabulary, transformer_model_name: str = "bert-base-cased", k_for_eval: int = 20, **kwargs
    ) -> None:
        super().__init__(vocab, **kwargs)
        self._k_for_eval = k_for_eval
    ...
    def forward(  # type: ignore
        ...
        k_value = 1 if self.training else self._k_for_eval
        # (batch_size, k, 2)
        k_best_spans = get_k_best_spans(span_start_logits, span_end_logits, k_value)
        # shape: (batch_size, 2)
        # best_spans = k_best_spans[:, 0, :]

        # Sum the span start score with the span end score to get an overall score for the span.
        # shape: (batch_size, k)
        best_span_scores = torch.gather(
            span_start_logits, 1, k_best_spans[:, :, 0]
        ) + torch.gather(span_end_logits, 1, k_best_spans[:, :, 1])
        #best_span_scores = best_span_scores.squeeze(1)

        best_span_probs = torch.gather(
            span_start_probs, 1, k_best_spans[:, :, 0]
        ) * torch.gather(span_end_probs, 1, k_best_spans[:, :, 1])
        ...
        output_dict = {
            ...
            "id": [d["id"] for d in metadata],
            "answers": [d["answers"] for d in metadata]
        }

        # Compute the loss.
        if answer_span is not None:
            output_dict["loss"] = self._evaluate_span(
                k_best_spans[:, 0, :], span_start_logits, span_end_logits, answer_span
            )
        ...
        if not self.training and metadata is not None:
            ...
            ) = self._collect_best_span_strings(k_best_spans, context_span, metadata, cls_index)
    ...
    def _collect_best_span_strings(
        ...
        """
        Collect the strings of the best predicted spans from the context metadata and
        update `self._per_instance_metrics`, which in the case of SQuAD v1.1 / v2.0
        includes the EM and F1 score.

        This returns a `Tuple[List[List[str]], torch.Tensor]`, where the `List[List[str]]` is the
        list of predicted answers for each instance in the batch, and the tensor is just the input
        tensor `k_best_spans` after adjustments so that each answer span corresponds to the
        ...
        for (metadata_entry, best_spans, cspan, cls_ind) in zip(
            ...
            if answers:
                best_span_strings_for_metric.append(best_span_strings[-1][0])
